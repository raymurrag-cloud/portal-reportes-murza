...
const json = readFileSync('C:/Users/murra/portal-reportes/CPNG.json', 'utf8');
JSON.parse(json);
console.log('JSON valido. Chars:', json.length);

const result = await db.execute({
  sql: 'UPDATE reportes SET contenido_json = ?, updated_at = CURRENT_TIMESTAMP, fecha_reporte = ? WHERE ticker = ?',
  args: [json, '2026-05-05', 'CPNG']
});
...
console.log('Verificacion:', JSON.stringify(verify.rows, null, 2));
