...
const ticker = 'MCD';
const empresa = "McDonald's Corporation";
const json = readFileSync('../MCD.json', 'utf8');

// Validate JSON before uploading
JSON.parse(json);
console.log('JSON valid, size:', json.length, 'chars');
...
if (existing.rows.length > 0) {
  await db.execute({
    ...
    args: [
      json,
      "Analisis financiero de McDonald's (MCD): modelo franquicia, FCF, margenes y veredicto de inversion.",
      ticker
    ]
  });
  console.log('UPDATED MCD - id:', existing.rows[0].id);
} else {
  await db.execute({
